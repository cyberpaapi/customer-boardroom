...
5. Move to seller planning and show each of the three seller QR codes to exactly one seller. Each invitation claims one seat. Sellers get identical working capital, but **no reaction scores, class average, customer budgets or preferences in round one**.
6. Sellers set prices and inventory for 15 fictional components across five categories. All combinations are compatible. Cases include motherboard, PSU and cooling. Prices cannot fall below wholesale cost.
7. Open the market. Customers buy one complete five-part PC, mixing sellers, or pass. Purchases are final; stock is validated atomically.
...
9. Sellers restock from scratch with the **same capital**. Customers have the **same original income and wishlist**. Run market two and compare profits, completion counts and wishlist matches.

**Profit = revenue − all stocked inventory costs.** Unsold stock has no recovery value. Higher revenue alone is not higher profit. Better information does not guarantee a winner: competition, prices, inventory and learning also affect results. This is a teaching simulation, not a causal study or a standardized reflex test. Device input/display latency can affect income.
...
`tests/game.test.js` covers scoring, privacy, uncapped joining, role limits, every action family, stock, budget checks, both rounds, Force End and Play Again. `tests/live.mjs` covers durable server actions, live observation, concurrent stock, resume credentials and idempotency. Set `TEST_API` to target an authorized test deployment.
